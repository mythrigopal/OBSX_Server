...
import org.obsplatform.infrastructure.core.exception.PlatformServiceUnavailableException;
import org.obsplatform.infrastructure.core.serialization.DatatableCommandFromApiJsonDeserializer;
import org.obsplatform.infrastructure.core.serialization.FromJsonHelper;
import org.obsplatform.infrastructure.core.serialization.JsonParserHelper;
import org.obsplatform.infrastructure.core.service.TenantAwareRoutingDataSource;
import org.obsplatform.infrastructure.dataqueries.api.DataTableApiConstant;
import org.obsplatform.infrastructure.dataqueries.data.DataTableValidator;
...
import org.obsplatform.infrastructure.dataqueries.exception.DatatableNotFoundException;
import org.obsplatform.infrastructure.dataqueries.exception.DatatableSystemErrorException;
...
public class ReadWriteNonCoreDataServiceImpl implements
	 ...
	 * READ (non-Javadoc)
	 * 
	 * @see #retrieveDatatableNames(java.lang.String)
	 */
	@Override
	public List<DatatableData> retrieveDatatableNames(final String appTable) {
		...
		final String sql = "select application_table_name, registered_table_name"
	 ...
	 * (non-Javadoc)
	 * 
	 * @see retrieveSingleDatatable(java.lang.String)
	 */
	@Override
	public DatatableData retrieveSingleDatatable(final String datatable) {
	 ...
	 * 
	 * @see
	 * #createDatatable(org.obsplatform.infrastructure.core.api.JsonCommand)
	 */
	@Transactional
	@Override
		...
		try {
			...
			if (multiRow) {
				sqlBuilder = sqlBuilder.append(
						"`id` BIGINT(20) NOT NULL AUTO_INCREMENT, ").append(
						"`" + fkColumnName + "` BIGINT(20) NOT NULL, ");
			} else {
				sqlBuilder = sqlBuilder.append("`" + fkColumnName
						+ "` BIGINT(20) NOT NULL, ");
			}
			...
			if (multiRow) {
				sqlBuilder = sqlBuilder
						...
						.append(", KEY `fk_" + apptableName.substring(2)
								+ "_id` (`" + fkColumnName + "`)")
			...
			if (realCause.getMessage().toLowerCase()
					.contains("duplicate column name")) {
				baseDataValidator.reset().parameter("name")
						.failWithCode("duplicate.column.name");
			} else if (realCause.getMessage().contains("Table")
					&& realCause.getMessage().contains("already exists")) {
				baseDataValidator.reset().parameter("datatableName")
						.value(datatableName)
						.failWithCode("datatable.already.exists");
			} else if (realCause.getMessage().contains("Column")
					&& realCause.getMessage().contains("big")) {
				baseDataValidator.reset().parameter("column")
						.failWithCode("length.too.big");
			} else if (realCause.getMessage().contains("Row")
					&& realCause.getMessage().contains("large")) {
				baseDataValidator.reset().parameter("row")
						.failWithCode("size.too.large");
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see #updateDatatable(java.lang.String,
	 * org.obsplatform.infrastructure.core.api.JsonCommand)
	 */
	@Transactional
	@Override
	public void updateDatatable(final String datatableName,final JsonCommand command) {
		...
		try {
			...
			if (!StringUtils.isBlank(apptableName)) {
				...
				// check for associated table
				final String oldApptableName = queryForApplicationTableName(datatableName);
				// if oldApptable,apptable's are different then go to change
				// fk..
				if (!StringUtils.equals(oldApptableName, apptableName)) {
					final String oldFKName = oldApptableName.substring(2)
							+ "_id";
					final String newFKName = apptableName.substring(2) + "_id";
					// final String actualAppTableName =
					// mapToActualAppTable(apptableName);
					final String oldConstraintName = datatableName
							.toLowerCase().replaceAll("\\s", "_")
							+ "_"
							+ oldFKName;
					final String newConstraintName = datatableName
							.toLowerCase().replaceAll("\\s", "_")
							+ "_"
							+ newFKName;
					StringBuilder sqlBuilder = new StringBuilder();
					...
					if (mapColumnNameDefinition.containsKey("id")) {
						sqlBuilder = sqlBuilder
								...
								.append("DROP FOREIGN KEY `fk_"
										+ oldConstraintName + "`,")
								.append("CHANGE COLUMN `" + oldFKName + "` `"
										+ newFKName + "` BIGINT(20) NOT NULL,")
								.append("ADD KEY `fk_" + newFKName + "` (`"
										+ newFKName + "`),")
								.append("ADD CONSTRAINT `fk_"
										+ newConstraintName + "` ")
								.append("FOREIGN KEY (`" + newFKName + "`) ")
								.append("REFERENCES `" + apptableName
										+ "` (`id`)");
					} else {
						sqlBuilder = sqlBuilder
								.append("ALTER TABLE `" + datatableName + "` ")
			...
			if (changeColumns == null && addColumns == null
					&& dropColumns == null) {
				return;
			}
			...
			if (dropColumns != null) {
				...
				for (final JsonElement column : dropColumns) {
					parseDatatableColumnForDrop(column.getAsJsonObject(),
							sqlBuilder, datatableName, constrainBuilder,
							codeMappings);
				}
			...
			if (realCause.getMessage().toLowerCase().contains("unknown column")) {
			...
			} else if (realCause.getMessage().toLowerCase()
					.contains("can't drop")) {
				baseDataValidator.reset().parameter("name")
						.failWithCode("does.not.exist");
			} else if (realCause.getMessage().toLowerCase()
				...
				baseDataValidator.reset().parameter("name")
						.failWithCode("column.already.exists");
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see #deleteDatatable(java.lang.String)
	 */
	@Transactional
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see #registerDatatable(java.lang.String, java.lang.String)
	 */
	@Transactional
	@Override
	public void registerDatatable(final String dataTableName,
			final String applicationTableName) {

		Integer category = DataTableApiConstant.CATEGORY_DEFAULT;

		final String permissionSql = this.getPermissionSql(dataTableName);
		this.registerAndPermissionDataTable(applicationTableName,
				dataTableName, category, permissionSql);

	}

	/**
	 * @param dataTableName
	 * @return inert into m_permission table sql query
	 */
	private String getPermissionSql(final String dataTableName) {
		final String createPermission = "'CREATE_" + dataTableName + "'";
		final String createPermissionChecker = "'CREATE_" + dataTableName
				+ "_CHECKER'";
	 ...
	 * @param dataTableName
	 * @param category
	 * @param permissionsSql
	 */
	@Transactional
	private void registerAndPermissionDataTable(
			final String applicationTableName, final String dataTableName,
		...
		try {
			...
			if (this.isSurveyCategory(category)) {
				this.jdbcTemplate
						.execute("insert into c_configuration (name, value, enabled ) values('"
								+ dataTableName + "', '0','0')");
			}
		 ...
		 ****/
			...
			if (realCause.getMessage().contains("Duplicate entry")) {
				throw new PlatformDataIntegrityException(
						...
						"Datatable `"
								+ dataTableName
								+ "` is already registered against an application table.",
	...
	/**
	 * @param appTable
	 *            check application table is exits or not
	 */
	private void validateAppTable(final String appTable) {
		...
		throw new PlatformDataIntegrityException(
				...
				"Invalid Application Table: " + appTable, "name", appTable);
	}

	/**
	 * @param column
	 * @param sqlBuilder
	 * @param constrainBuilder
	 * @param dataTableNameAlias
	 * @param codeMappings
	 * @param isConstraintApproach
	 *            prepare create query
	 */
	private void parseDatatableColumnObjectForCreate(final JsonObject column,
			StringBuilder sqlBuilder, final StringBuilder constrainBuilder,
		...
		final String type = (column.has("type")) ? column.get("type")
				.getAsString().toLowerCase() : null;

		final Integer length = (column.has("length")) ? column.get("length")
				.getAsInt() : null;

		final Boolean mandatory = (column.has("mandatory")) ? column.get(
				"mandatory").getAsBoolean() : false;

		final String code = (column.has("code")) ? column.get("code")
				.getAsString() : null;
		...
		if (StringUtils.isNotBlank(code)) {
			...
			if (isConstraintApproach) {
				codeMappings.put(dataTableNameAlias + "_" + name,
						this.codeReadPlatformService.retriveCode(code)
								.getCodeId());
				constrainBuilder.append(", CONSTRAINT `fk_")
						.append(dataTableNameAlias).append("_").append(name)
						.append("` ").append("FOREIGN KEY (`" + name + "`) ")
						.append("REFERENCES `").append(CODE_VALUES_TABLE)
						.append("` (`id`)");
			} else {
		...
		if (type != null) {
			if (type.equalsIgnoreCase("String")) {
				...
				sqlBuilder = sqlBuilder.append("(20)");
			}
		}
		if (mandatory != null) {
			if (mandatory) {
	 ...
	 */
	private void parseDatatableColumnForUpdate(
			...
			final StringBuilder constrainBuilder,
			final Map<String, Long> codeMappings,
			final List<String> removeMappings,
			final boolean isConstraintApproach) {

		String name = (column.has("name")) ? column.get("name").getAsString()
				: null;
		final String lengthStr = (column.has("length")) ? column.get("length")
				.getAsString() : null;
		Integer length = (StringUtils.isNotBlank(lengthStr)) ? Integer
				.parseInt(lengthStr) : null;
		String newName = (column.has("newName")) ? column.get("newName")
				.getAsString() : name;
		final Boolean mandatory = (column.has("mandatory")) ? column.get(
				"mandatory").getAsBoolean() : false;
		final String after = (column.has("after")) ? column.get("after")
				.getAsString() : null;
		final String code = (column.has("code")) ? column.get("code")
				.getAsString() : null;
		final String newCode = (column.has("newCode")) ? column.get("newCode")
				.getAsString() : null;
			...
			if (!StringUtils.equalsIgnoreCase(code, newCode)
				...
				if (StringUtils.equalsIgnoreCase(code, newCode)) {
					...
					if (codeId > 0) {
						...
						constrainBuilder.append(", DROP FOREIGN KEY `fk_")
								.append(dataTableNameAlias).append("_")
								.append(name).append("` ");
						codeMappings.put(dataTableNameAlias + "_" + newName,
								(long) codeId);
						constrainBuilder.append(",ADD CONSTRAINT `fk_")
								.append(dataTableNameAlias).append("_")
								.append(newName).append("` ")
								.append("FOREIGN KEY (`" + newName + "`) ")
								.append("REFERENCES `")
					...
					if (newCode != null) {
						codeMappings.put(
								dataTableNameAlias + "_" + newName,
								this.codeReadPlatformService.retriveCode(
										newCode).getCodeId());
						if (code == null
								|| !StringUtils.equalsIgnoreCase(name, newName)) {
							constrainBuilder.append(",ADD CONSTRAINT `fk_")
		...
		if (!mapColumnNameDefinition.containsKey(name)) {
			throw new PlatformDataIntegrityException(
					"error.msg.datatable.column.missing.update.parse",
		...
		if (length == null && type.toLowerCase().equals("varchar")) {
			length = mapColumnNameDefinition.get(name).getColumnLength()
					.intValue();
		}

		sqlBuilder = sqlBuilder.append(", CHANGE `" + name + "` `" + newName
				+ "` " + type);
	...
	private void parseDatatableColumnForDrop(final JsonObject column,
		...
		final String datatableAlias = datatableName.toLowerCase().replaceAll(
				"\\s", "_");
		final String name = (column.has("name")) ? column.get("name")
				.getAsString() : null;
		sqlBuilder = sqlBuilder.append(", DROP COLUMN `" + name + "`");
		final StringBuilder findFKSql = new StringBuilder();
		findFKSql.append("SELECT count(*)")
				.append("FROM information_schema.TABLE_CONSTRAINTS i")
				.append(" WHERE i.CONSTRAINT_TYPE = 'FOREIGN KEY'")
				.append(" AND i.TABLE_SCHEMA = DATABASE()")
				.append(" AND i.TABLE_NAME = '").append(datatableName)
				.append("' AND i.CONSTRAINT_NAME = 'fk_")
		...
		if (count > 0) {
			...
			constrainBuilder.append(", DROP FOREIGN KEY `fk_")
					.append(datatableAlias).append("_").append(name)
					.append("` ");
		}
	}
	...
	private void registerColumnCodeMapping(final Map<String, Long> codeMappings) {
		if (codeMappings != null && !codeMappings.isEmpty()) {
			...
			int i = 0;
			for (final Map.Entry<String, Long> mapEntry : codeMappings
					.entrySet()) {
				addSqlList[i++] = "insert into x_table_column_code_mappings (column_alias_name, code_id) values ('"
	...
	private void deleteColumnCodeMapping(final List<String> columnNames) {
		if (columnNames != null && !columnNames.isEmpty()) {
			...
			for (final String columnName : columnNames) {
				deleteSqlList[i++] = "DELETE FROM x_table_column_code_mappings WHERE column_alias_name='"
						+ columnName + "';";
			}
	 ...
	 * 
	 * @see
	 * #registerDatatable(org.obsplatform.infrastructure.core.api.JsonCommand,
	 * java.lang.String)
	 */
	@Transactional
	@Override
	...
	private Integer getCategory(final JsonCommand command) {
		Integer category = command
				.integerValueOfParameterNamedDefaultToNullIfZero(DataTableApiConstant.categoryParamName);
		if (category == null)
	 ...
	 * (non-Javadoc)
	 * 
	 * @see #deregisterDatatable(java.lang.String)
	 */
	@Transactional
	@Override
	public void deregisterDatatable(final String datatable) {
		final String permissionList = "('CREATE_" + datatable + "', 'CREATE_"
				...
				+ datatable + "', 'UPDATE_" + datatable + "_CHECKER', 'DELETE_"
				+ datatable + "', 'DELETE_" + datatable + "_CHECKER')";
		...
		final String deletePermissionsSql = "delete from m_permission where code in "
				+ permissionList;

		final String deleteRegisteredDatatableSql = "delete from x_registered_table where registered_table_name = '"
				+ datatable + "'";

		final String deleteFromConfigurationSql = "delete from c_configuration where name ='"
				+ datatable + "'";
	...
	public CommandProcessingResult createPPIEntry(final String dataTableName,
		...
		try {
			...
			final CommandProcessingResult commandProcessingResult = checkMainResourceExistsWithinScope(
					appTable, appTableId);
	...
	/**
	 * @param name
	 * @return check data table is there in x_registered_table
	 */
	private boolean isRegisteredDataTable(final String name) {
		...
		final String isRegisteredDataTable = this.jdbcTemplate.queryForObject(
				sql, String.class, new Object[] { name });
	...
	private void assertDataTableExists(final String datatableName) {
		...
		final String dataTableExistsString = this.jdbcTemplate.queryForObject(
				sql, String.class, new Object[] { datatableName });
		final boolean dataTableExists = new Boolean(dataTableExistsString);
		if (!dataTableExists) {
			throw new PlatformDataIntegrityException(
					"error.msg.invalid.datatable", "Invalid Data Table: "
	...
	/*
	 * (non-Javadoc) READ DATA OPERATIONS
	 * 
	 * @see #retrieveDataTableGenericResultSet(java.lang.String, java.lang.Long,
	...
	public GenericResultsetData retrieveDataTableGenericResultSet(
		...
		// (when updating)
		if (id == null) {
			sql = sql + "select * from `" + dataTableName + "` where "
					+ getFKField(appTable) + " = " + appTableId;
		} else {
			sql = sql + "select * from `" + dataTableName + "` where id = "
					+ id;
		}
	...
	private int getCodeIdForColumn(final String dataTableNameAlias,
			final String name) {
		final StringBuilder checkColumnCodeMapping = new StringBuilder();
		checkColumnCodeMapping
				.append("select ccm.code_id from x_table_column_code_mappings ccm where ccm.column_alias_name='")
				.append(dataTableNameAlias).append("_").append(name)
				.append("'");
		int codeId = 0;
		try {
			codeId = this.jdbcTemplate.queryForInt(checkColumnCodeMapping
					.toString());
	 ...
	 * @param sql
	 * @return
	 * fill row data with columns
	 */
	private List<ResultsetRowData> fillDatatableResultSetDataRows(
			final String sql) {
		...
		while (rs.next()) {
			...
			final ResultsetRowData resultsetDataRow = ResultsetRowData
					.create(columnValues);
	...
	/* (non-Javadoc)
	 * @see #createNewDatatableEntry(java.lang.String, java.lang.Long, org.obsplatform.infrastructure.core.api.JsonCommand)
	 */
	@Transactional
	@Override
	public CommandProcessingResult createNewDatatableEntry(
			final String dataTableName, final Long appTableId,
	...
	@Transactional
	@Override
	public CommandProcessingResult updateDatatableEntryOneToOne(
			final String dataTableName, final Long appTableId,
			final JsonCommand command) {
	...
	@Transactional
	@Override
	public CommandProcessingResult updateDatatableEntryOneToMany(
			final String dataTableName, final Long appTableId,
	...
	private CommandProcessingResult updateDatatableEntry(
			final String dataTableName, final Long appTableId,
			final Long datatableId, final JsonCommand command) {
		...
		final GenericResultsetData grs = retrieveDataTableGenericResultSetForUpdate(
				appTable, dataTableName, appTableId, datatableId);
		...
		if (!changes.isEmpty()) {
			...
			final String sql = getUpdateSql(dataTableName, pkName, pkValue,
					changes);
	...
	@Transactional
	@Override
	public CommandProcessingResult deleteDatatableEntries(
			final String dataTableName, final Long appTableId) {
		...
		final String deleteOneToOneEntrySql = getDeleteEntriesSql(
				dataTableName, getFKField(appTable), appTableId);

		final int rowsDeleted = this.jdbcTemplate
				.update(deleteOneToOneEntrySql);
		if (rowsDeleted < 1) {
			throw new DatatableNotFoundException(dataTableName, appTableId);
		}
	...
	@Transactional
	@Override
	public CommandProcessingResult deleteDatatableEntry(
			final String dataTableName, final Long appTableId,
	...
	private GenericResultsetData retrieveDataTableGenericResultSetForUpdate(
			final String appTable, final String dataTableName,
	 ...
	 * @return
	 */
	private CommandProcessingResult checkMainResourceExistsWithinScope(
			final String appTable, final Long appTableId) {
		...
		return new CommandProcessingResultBuilder().withOfficeId(officeId)
				.withGroupId(groupId).withClientId(clientId).build();
	}
	...
	private String dataScopedSQL(final String appTable, final Long appTableId) {
		...
		if (appTable.equalsIgnoreCase("m_client")) {
			scopedSQL = "select o.id as officeId, c.group_id as groupId, c.id as clientId from m_client c "
		...
		if (appTable.equalsIgnoreCase("m_group")
			...
			scopedSQL = "select o.id as officeId, g.id as groupId, null as clientId from m_group g "
		...
		if (appTable.equalsIgnoreCase("m_office")) {
			scopedSQL = "select o.id as officeId, null as groupId, null as clientId from m_office o "
	 ...
	 * @param queryParams
	 * @return
	 * prepare query for create data for datatable
	 */
	private String getAddSql(
			final List<ResultsetColumnHeaderData> columnHeaders,
			final String datatable, final String fkName, final Long appTableId,
			final Map<String, String> queryParams) {

		final Map<String, String> affectedColumns = getAffectedColumns(
				columnHeaders, queryParams, fkName);
		...
		for (final String key : affectedColumns.keySet()) {
			...
			if (StringUtils.isEmpty(pValue)) {
				...
				pValueWrite = singleQuote
						+ this.genericDataService.replace(pValue, singleQuote,
	 ...
	 * @return
	 */
	public String getAddSqlWithScore(
			final List<ResultsetColumnHeaderData> columnHeaders,
			final String datatable, final String fkName, final Long appTableId,
			final Map<String, String> queryParams) {
		...
		String vaddSql = "insert into `"
				...
				+ " as id"
				+ selectColumns
				+ " , ( SELECT SUM( code_score ) FROM m_code_value WHERE m_code_value.id IN ("
				+ scoresId + " ) ) as score";
	...
	private String getUpdateSql(final String datatable,
	...
	private Map<String, Object> getAffectedAndChangedColumns(
			...
			final Map<String, String> queryParams, final String fkName) {
	...
	public Map<String, String> getAffectedColumns(
			final List<ResultsetColumnHeaderData> columnHeaders,
		...
		for (final String key : keys) {
			...
			if (!((key.equalsIgnoreCase("id"))
				...
				queryParamColumnUnderscored = this.genericDataService.replace(
						key, space, underscore);
				for (final ResultsetColumnHeaderData columnHeader : columnHeaders) {
					if (notFound) {
						columnHeaderUnderscored = this.genericDataService
	...
	private String validateColumn(final ResultsetColumnHeaderData columnHeader,
			final String pValue, final String dateFormat,
		...
		if (StringUtils.isNotEmpty(paramValue)) {
			...
			if (columnHeader.isDateDisplayType()) {
				final LocalDate tmpDate = JsonParserHelper.convertFrom(
						paramValue, columnHeader.getColumnName(), dateFormat,
				...
				final Integer tmpInt = this.helper.convertToInteger(paramValue,
				...
				final BigDecimal tmpDecimal = this.helper.convertFrom(
						paramValue, columnHeader.getColumnName(),
						clientApplicationLocale);
				if (tmpDecimal == null) {
					paramValue = null;
				} else {
				...
				if (paramValue.length() > columnHeader.getColumnLength()) {
					final ApiParameterError error = ApiParameterError
							.parameterError(
									...
									"The column `"
											+ columnHeader.getColumnName()
											+ "` exceeds its defined max-length ",
	...
	private String getDeleteEntriesSql(final String datatable,
			final String FKField, final Long appTableId) {
	...
	private String getDeleteEntrySql(final String datatable,
			final Long datatableId) {
	...
	private boolean notTheSame(final String currValue, final String pValue,
		...
		if ("DECIMAL".equalsIgnoreCase(colType)) {
			final BigDecimal currentDecimal = BigDecimal.valueOf(Double
					.valueOf(currValue));
			final BigDecimal newDecimal = BigDecimal.valueOf(Double
					.valueOf(pValue));
